...
/home/lihaifei/lihaifei/.conda/envs/myenv/bin/python main.py 12 > /mnt/PRESKY/user/lihaifei/yuchen/46/data_process_code/12.log

# 对插值数据进行订正
sh /mnt/PRESKY/user/lihaifei/yuchen/46/corr_code/crontab_corr_12.sh

#current_time=`date -d "1 days ago" "+%Y-%m-%d %H:%M"`
#time_param=`date -d "$current_time" +%s`
#input_yyyymmdd_data=`date -d @$time_param "+%Y-%m-%d"`
#input_yyyytmmddHH_data=`date -d @$time_param "+%Y-%m-%d 12"`
...
#python Diagnose_046_NC_ec.py $input_yyyytmmddHH_data > log/Diagnose_046_NC_ec_12.log
