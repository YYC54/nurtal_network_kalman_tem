...
class Thunder(XXX_M):
    ...
    def feature(a):
        ...
        try:
            ...
            a['time'] = pd.to_datetime(a['time'], format="ISO8601")
        ...
        if len(self.dataframe):
            for func in self.methods:
                if func == 'neural_network':
                    path = config.get('Thunder_Path', f'{func}_path')
            ...
            if 'ensemble' in self.methods:
                # 应用函数，生成新列
                self.dataframe['ensemble'] = self.dataframe[['xgboost', 'storm', 'random_forest']].apply(
                    self.most_frequent, axis=1)
            else:
                pass
        else:
            raise ('输入有误！')
...
if __name__ == "__main__":
    ...
    # })
    #
    # train_e = Thunder(element_model='rf', params={
    #     'n_estimators': 800,
    ...
    #     data['stime'] = pd.to_datetime(data['stime'])
    #     data['month'] = data['stime'].dt.month
    # df = data.query('month >=5 and month<10')
    # df_1 = df.query('td == 1')
    # df_0 = df.query('td == 0')
