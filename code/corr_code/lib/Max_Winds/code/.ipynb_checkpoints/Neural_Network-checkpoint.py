...
class Neural_Network(object):
    ...
    def data_loader(self, df):
        ...
        if self.type == "train":
            ...
            with open(config["maxWins"]["scaler_path"], "wb") as f:
            ...
            with open(config["maxWins"]["scaler_path"], "rb") as f:
    ...
    def train_model(self):
        ...
        for sw_max_id in self.sw_max_ids:
            ...
            plt.savefig(
                os.path.join(
                    config["maxWins"]["loss_fig_path"],
                    f"loss_history_sw_max{sw_max_id}.png",
                )
            )
            model.save(
                os.path.join(
                    config["maxWins"]["save_model_path"], f"Max_Wind_{sw_max_id}.h5"
                )
            )
