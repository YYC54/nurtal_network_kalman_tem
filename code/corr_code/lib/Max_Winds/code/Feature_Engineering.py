...
class Feature_Engineering(object):
    ...
    def select_features(self, df, thus):
        if self.type == "train":
            ...
            df_corr_1_res.to_csv(config.get('MaxWins_Path', 'corr_sw_max1_path'))

            df_corr_2_res = pd.DataFrame(df_corr.corr()["sw_max2"])
            df_corr_2_res.sort_values("sw_max2", ascending=False)
            df_corr_2_res.to_csv(config.get('MaxWins_Path', 'corr_sw_max2_path'))
            ...
            # 将筛选的特征名存储以便推理使用
            with open(config.get('MaxWins_Path', 'selected_feats_path'), "wb") as f:
                pickle.dump(self.CONSTANT_FEATS + feats, f)

        else:
            with open(config.get('MaxWins_Path', 'selected_feats_path'), "rb") as f:
