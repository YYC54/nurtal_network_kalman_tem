...
def save_MaxWins(corresDateFrame, save_corr_path):
    ...
    for sw_max_col in sw_max_cols:
        ...
        new_df = df[['time', 'time_order', 'dtime', 'id', 'lon', 'lat', 'stime', '北京时间',
                     f'sw_max{sw_max_col}_pred_nn', f'sw_max{sw_max_col}_pred_kalman',
                     f'sw_max{sw_max_col}_pred_ensemble']].copy()

        # 重命名列名
        new_df.rename(columns={f'sw_max{sw_max_col}_pred_nn': 'nm',
                               f'sw_max{sw_max_col}_pred_kalman': 'kf',
                               f'sw_max{sw_max_col}_pred_ensemble': 'co'}, inplace=True)
