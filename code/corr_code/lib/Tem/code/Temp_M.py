...
import os
Logger = Logger(path="./log/tem-corr.log",
                clevel=logging.ERROR, Flevel=logging.DEBUG)
...
class Temp_M(object):
    ...
    def __init__(self, factime, dataframe, element, methods):
        ...
        self.scaler_path = config.get('Tem_Path', 'scaler_path')
        self.model_tem = config.get('Tem_Path', 'save_model_path')
        self.train_data_path = config.get('Tem_Path', 'train_data_path')
    ...
    def _feature(self):
        ...
        try:
            self.train_data["time"] = pd.to_datetime(self.train_data["time"], format="%Y/%m/%d %H:%M:%S")
        except Exception as e:
            self.dataframe["time"] = pd.to_datetime(
                self.dataframe["time"], format="ISO8601"
            )
    ...
    def _pred(self):
        ...
        # filtered_predictions_df.to_csv(s
        #             self.final_csv,
        #             index=False)
        filtered_predictions_df.rename(columns={'tem_neural_network': 'nm', 'tem_kalman_filter': 'kf',
